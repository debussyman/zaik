...
Accepted canonical observations fan out through the monitored `Zaik.Home.EventBus`; stale and duplicate reports never emit events. The autonomy engine subscribes in shadow-only mode and coalesces relevant changes using injected timers; canary and active execution remain rejected.
