...
Accepted canonical observations fan out through the monitored `Zaik.Home.EventBus`; stale and duplicate reports never emit events. The autonomy engine can subscribe behind disabled-by-default shadow configuration and coalesce relevant changes using injected timers.
