...
| `Zaik.Home.StagedPlanScheduler` | Mirror-only virtual-time wakeups, automatic polling, and reconstruction of durable waiting or interrupted running plans after scheduler restart. | Add observation-triggered wakeups and watchdog diagnostics while retaining the production execution barrier. |
