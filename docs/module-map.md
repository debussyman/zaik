...
| `Zaik.Home.StagedPlanCoordinator` | Supervised ledger-protected stage execution, separate condition/verification waits, convergence-gated advancement, policy-gated low-risk retries, cooperative cancellation, and durable mirror-only resume across scheduler and ledger restart. | Accumulate replay evidence before considering any production binding. |
