...
| `Zaik.Home.StagedPlanStore` | Durable prepared/waiting/running/completed/cancelled/expired staged-plan lifecycle with exact-ID operator audit, stage checkpoints, bounded run-event diagnostics, restart recovery, and terminal retention. | Keep production execution unavailable until promotion gates exist. |
| `Zaik.Home.StagedPlanCoordinator` | Supervised ledger-protected stage execution, canonical reobservation, condition waits, timeout cancellation, and durable resume restricted to isolated mirror executors. | Add staged retry policy integration before considering any production binding. |
| `Zaik.Home.StagedPlanScheduler` | Mirror-only virtual-time and dependency-filtered canonical-observation wakeups, automatic polling, reconstruction after restart, and durable attempt/timeout/task-exit diagnostics. | Add alert thresholds and staged retry policy integration while retaining the production execution barrier. |
