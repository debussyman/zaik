...
| `Zaik.Home.StagedPlanStore` | Durable prepared/waiting/running/completed/cancelled/expired staged-plan lifecycle with exact-ID operator audit, stage checkpoints, restart recovery, and bounded terminal retention. | Keep production execution unavailable until promotion gates exist. |
...
| `Zaik.Home.StagedPlanScheduler` | Mirror-only virtual-time and dependency-filtered canonical-observation wakeups, automatic polling, and reconstruction of durable waiting or interrupted running plans after scheduler restart. | Add watchdog diagnostics while retaining the production execution barrier. |
