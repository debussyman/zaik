...
| `Zaik.Home.World` | Projects adapter payloads into capability-filtered current state with explicit observation freshness classes and stable exact-fact snapshot IDs. | Keep as the ordinary home reasoning boundary; receipt time must never become observation time. |
| `Zaik.Home.Query` / `EntityResolver` | Versioned shared query normalization and deterministic ID/name/area/alias matching across current state, history, policies, skills, and presets. | Expand metamorphic vocabulary only with invariant tests and a declared version change. |
| `Zaik.Home.WorldContract` | Versioned runtime-discovered canonical entity/capability, freshness, snapshot, desired-state, confidence, provenance, and calibration schemas with stable state-free fingerprints. | Preserve explicit schema versions and update contract tests whenever semantics change. |
...
| `Zaik.Home.Autonomy.RolloutGate` / `RolloutStore` | Repeated mirror plus production-shadow readiness evidence, exact allowlists, operator approval, supersession, and rollback for a future physical trial. | Evidence-only: never enable canary/active mode or expose execution authority. |
| `Zaik.Home.Autonomy.CanaryTrial` | Inert proposal plus exact operator confirmation for one allowlisted action that is revalidated against a fresh policy decision before normal explicit execution. | Keep one-action, operator-authorized, idempotent, verified, and outside continuous autonomy modes. |
