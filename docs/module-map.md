...
| `Zaik.Home.StagedPlanWatchdog` | Read-only code-owned thresholds for missed wakeups, stuck evaluations, and repeated failures over active plans and the bounded run journal. | Connect typed issues to operator alert delivery without granting execution authority. |
