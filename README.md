...
- **One house brain** — Telegram and optional Signal ingress normalize into one bounded `Zaik.AgentChat` tool loop. Semantic goal paraphrases retrieve versioned skill context, while the model selects the goal/evidence workflow—there is no phrase-to-device command branch. Model-authored home skills can only become validated inert proposals; an exact operator confirmation is required before an atomic skill-file write.
