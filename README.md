...
- **One house brain** — Telegram and optional Signal ingress normalize into one bounded `Zaik.AgentChat` tool loop. Model-authored home skills can only become validated inert proposals; an exact operator confirmation is required before an atomic skill-file write.
