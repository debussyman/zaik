...
- **Typed world model** — models see entities, capabilities, room context, history, occupancy, modes, presets, and evidence under a versioned runtime-discovered contract—not raw MQTT topics. Per-entity adapter calibration is separately persisted with independent evidence, operator identity, semantic fingerprints, and append-only revisions.
