defmodule Zaik.Application do
  ...
  def children do
    [
      ...
      domain_child(:home, autonomy_mode_store_child()),
  ...
  defp occupancy_transition_store_child do
    config = Zaik.Home.Autonomy.Engine.config()

    if config.enabled do
      {Zaik.Home.OccupancyTransitionStore, db_path: config.decision_db_path}
    end
  end
  ...
  defp autonomy_mode_store_child do
    config = Zaik.Home.Autonomy.Engine.config()

    if config.enabled do
      {Zaik.Home.Autonomy.ModeStore, db_path: config.decision_db_path}
    end
  end
  ...
  defp autonomy_scope_mode_store_child do
    config = Zaik.Home.Autonomy.Engine.config()
  ...
  defp staged_plan_store_child do
    config = Zaik.Home.Autonomy.Engine.config()

    if config.enabled do
      {Zaik.Home.StagedPlanStore, db_path: config.decision_db_path}
    end
  end
