defmodule Zaik.Tools.RegistryTest do
  ...
  test "fingerprints runtime tool contracts deterministically" do
    first = Zaik.Tools.Registry.fingerprint(modules: [HotTool])
    second = Zaik.Tools.Registry.fingerprint(modules: [HotTool])

    assert first == second
    refute first == Zaik.Tools.Registry.fingerprint()
  end
  ...
  test "default registry exposes typed state and existing compatibility tools" do
    ...
    assert "get_home_history" in names
    assert "get_home_action_status" in names
